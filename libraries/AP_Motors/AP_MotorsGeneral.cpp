...
#include "AP_MotorsGeneral.h"

extern const AP_HAL::HAL& hal;

// init
void AP_MotorsGeneral::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
void AP_MotorsGeneral::set_update_rate(uint16_t speed_hz)
{
...
void AP_MotorsGeneral::set_frame_class_and_type(motor_frame_class frame_class, motor_frame_type frame_type)
{
...
void AP_MotorsGeneral::output_to_motors()
{
    int8_t i;
...
uint16_t AP_MotorsGeneral::get_motor_mask()
{
...
void AP_MotorsGeneral::output_armed_stabilizing()
{
    ...
    for (i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        Vector3f torque = _dir[i].cross(_loc[i]);
        if (motor_enabled[i]) {
            // calculate the thrust outputs for roll and pitch
            _thrust_rpyt_out[i] = roll_thrust * torque.x + pitch_thrust * torque.y;
            ...
            if (!is_zero(torque.z) && (!_thrust_boost || i != _motor_lost_index)){
                if (is_positive(yaw_thrust * torque.z)) {
                    yaw_allowed = MIN(yaw_allowed, fabsf(MAX(1.0f - (throttle_thrust_best_rpy + _thrust_rpyt_out[i]), 0.0f)/torque.z));
                } else {
                    yaw_allowed = MIN(yaw_allowed, fabsf(MAX(throttle_thrust_best_rpy + _thrust_rpyt_out[i], 0.0f)/torque.z));
                }
            }
        }
    }
    ...
    for (i = 0; i < AP_MOTORS_MAX_NUM_MOTORS; i++) {
        ...
        Vector3f torque = _dir[i].cross(_loc[i]);
        // in general, shaft_torque is actual motor torque, which is proportional to motor input power
        // _yaw_factor represents rotation direction: +1 for CCW, -1 for CW
        Vector3f shaft_torque = _dir[i] * _yaw_factor[i];
//        hal.console->printf("storque: %f %f %f\n", shaft_torque.x, shaft_torque.y, shaft_torque.z);
        torque += shaft_torque;
        
        if (motor_enabled[i]) {
            _thrust_rpyt_out[i] = _thrust_rpyt_out[i] + yaw_thrust * torque.z;
...
void AP_MotorsGeneral::check_for_failed_motor(float throttle_thrust_best_plus_adj)
{
...
// todo: exercise tilt servo(s)
void AP_MotorsGeneral::output_test_seq(uint8_t motor_seq, int16_t pwm)
{
...
//  pwm value is an actual pwm value that will be output, normally in the range of 1000 ~ 2000
bool AP_MotorsGeneral::output_test_num(uint8_t output_channel, int16_t pwm)
{
    if (!armed()) {
        return false;
    }
...
// add_motor
void AP_MotorsGeneral::add_motor_raw(int8_t motor_num, float roll_fac, float pitch_fac, float yaw_fac, uint8_t testing_order)
{
    ...
    if (motor_num >= 0 && motor_num < AP_MOTORS_MAX_NUM_MOTORS) {
        ...
        // set roll, pitch and yaw factors assuming planar, vertical motor geometry
        // use normalized arm length of 1 to back out motor location from roll_fac and pitch_fac
        // x distance from CG determines pitch moment
        // y distance from CG determines roll moment
        _loc[motor_num].x = pitch_fac;
        _loc[motor_num].y = -roll_fac;
        _loc[motor_num].z = 0.0f;
        _yaw_factor[motor_num] = yaw_fac;
        _dir[motor_num].x = 0.0f;
        _dir[motor_num].y = 0.0f;
        _dir[motor_num].z = 1.0f;
        _dir[motor_num].normalize();
...
void AP_MotorsGeneral::add_motor(int8_t motor_num, float angle_degrees, float yaw_factor, uint8_t testing_order)
{
    add_motor(motor_num, angle_degrees, angle_degrees, yaw_factor, testing_order);
}

// add_motor using position and prop direction. Roll and Pitch factors can differ (for asymmetrical frames)
void AP_MotorsGeneral::add_motor(int8_t motor_num, float roll_factor_in_degrees, float pitch_factor_in_degrees, float yaw_factor, uint8_t testing_order)
{
    add_motor_raw(
        motor_num,
...
void AP_MotorsGeneral::remove_motor(int8_t motor_num)
{
    // ensure valid motor number is provided
    if (motor_num >= 0 && motor_num < AP_MOTORS_MAX_NUM_MOTORS) {
...
void AP_MotorsGeneral::setup_motors(motor_frame_class frame_class, motor_frame_type frame_type)
{
        ...
        case MOTOR_FRAME_TRI:
            switch(frame_type) {
                case MOTOR_FRAME_TYPE_PLUS:
                    ...
                    add_motor(AP_MOTORS_MOT_2,  -45, 0, 3);
                    add_motor(AP_MOTORS_MOT_4,  180, 0, 2);
                    break;
                case MOTOR_FRAME_TYPE_PLUSREV:
                    add_motor(AP_MOTORS_MOT_1,  135, 0, 1);
                    add_motor(AP_MOTORS_MOT_2, -135, 0, 2);
                    add_motor(AP_MOTORS_MOT_4,    0, 0, 3);
                    break;
                default:
                    // tri frame class does not support this frame type
                    success = false;
                    break;
            }
...
// This assumes that motors are paired on the unit circle, so the maximum 
// magnitude for xxx_factor is 1.0.  Note that for a very asymmetric frame,
// the maximum roll_factor magnitude might be very different from that of
// pitch_factor; This makes it clear that the actual magnitude of these
// factors is somewhat arbitrary, with the intent that max torque demand is
// 0.5 on all axes regardless of specific geometry.
#if 0
// normalizes the roll, pitch and yaw factors so maximum magnitude is 0.5
void AP_MotorsGeneral::normalise_rpy_factors()
{
    float roll_fac = 0.0f;
 ...
 */
void AP_MotorsGeneral::output_motor_mask(float thrust, uint8_t mask, float rudder_dt)
{
...
*/
void AP_MotorsGeneral::thrust_compensation(void)
{
