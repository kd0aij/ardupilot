...
#            raise NotAchievedException("Extra parameters in XML")
    ...
    def get_function_code_from_name_and_description(self, param_name, description):
        ...
        function_code = self.htree[param_name]
        code = None
            ...
            if code == None:
                ...
                for i in range(len(v)):
                    if v[i].text.find(description) >= 0:
                        if code != None:
                            raise NotAchievedException("ambiguous function description '%s' for param %s" % (description, param_name))
                        code = v[i].get('code')
            if code != None:
                return float(code)
            else:
                raise NotAchievedException("no matching function description '%s' for param %s" % (description, param_name))


    def set_param_from_description(self, param_name, description):
        value = self.get_function_code_from_name_and_description(param_name, description)
    ...
    def set_bitfield_param(self, param_name, description, overwrite=False):
        bitnumber = self.get_bitfield_number_from_description(param_name, description)
        value = (1 << bitnumber)
        if not overwrite:
            self.progress("or'ing bit %d into %s" % (bitnumber, param_name))
            value |= (int)(self.get_parameter(param_name))
            self.set_parameter(param_name, value)
        else:
            self.progress("overwriting bit %d into %s" % (bitnumber, param_name))
            self.set_parameter(param_name, value)
