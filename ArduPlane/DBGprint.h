...
public:
    ...
    static void dbgprint(const Otype type, const uint32_t interval_ms, const uint32_t max_prints, const char *fmt, ...) FMT_PRINTF(4, 5) {
        void *call_loc = __builtin_return_address(0);
        va_list ap;
        va_start(ap, fmt);
        output_txt(call_loc, type, interval_ms, max_prints, fmt, ap);
        va_end(ap);
    }
...
private:
    ...
    static void output_txt(void *call_loc, const Otype type, const uint32_t interval_ms, const uint32_t max_prints, const char *fmt, va_list ap) {
