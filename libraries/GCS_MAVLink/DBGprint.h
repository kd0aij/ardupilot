...
public:
    ...
    static void printf(const OType otype, const uint32_t interval_ms, const uint32_t max_prints, const char *fmt, ...) FMT_PRINTF(4, 5);

    // print only when value changes
    template <typename T>
    static void watch(const OType otype, T value, const uint32_t max_prints, const char *fmt, ...) {
