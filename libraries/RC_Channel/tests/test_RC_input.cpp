...
public:
    ...
    static float original_norm_input(int16_t radio_min, int16_t radio_max, 
    ...
    static float original_norm_input_dz(int16_t dead_zone, int16_t radio_min, int16_t radio_max,
                                        int16_t radio_trim, int16_t radio_in, bool reversed)
    {
...
TEST(RCInputTest, norm_input_set_radio_in)
{
    ...
    do {
        ...
        printf("reversed: %d, dead_zone: %d\n", reversed, dead_zone);
        // compare with original norm_input over the pwm range [pwm_min, pwm_max]
        for (int16_t pwm=pwm_min; pwm<=pwm_max; pwm+=1) {
            orig_norm_input = GoldStandard::original_norm_input(pwm_min, pwm_max, pwm_trim, pwm, reversed);
            orig_norm_input_dz = GoldStandard::original_norm_input_dz(dead_zone, pwm_min, pwm_max, pwm_trim, pwm, reversed);
