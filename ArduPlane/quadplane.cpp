...
#define FRAME_CLASS_OLD_IDX 30
    ...
    // @Bitmask: 0:LevelTransition,1:AllowFWTakeoff,2:AllowFWLand,3:Respect takeoff frame types,4:Use a fixed wing approach for VTOL landings,5:Use QRTL instead of QLAND for failsafe when in VTOL modes,6:Use idle governor in MANUAL,7:enable AirMode if arming by aux switch
...
{    
    ...
    if ((throttle_in <= 0) && (plane.air_mode == AirMode::OFF)) {
...
void QuadPlane::set_armed(bool armed)
{
    ...
    // If OPTION_AIRMODE is selected and no AIRMODE switch is assigned and
    // rudder arming is disabled (don't want vehicle to yaw when disarming with rudder)
    if ((options & OPTION_AIRMODE) && 
        (rc().find_channel_for_option(RC_Channel::AUX_FUNC::AIRMODE) == nullptr) &&
        (plane.arming.get_rudder_arming_type() == AP_Arming::RudderArming::IS_DISABLED)) {
        // automatically enable/disable AirMode on arming/disarming
        if (armed) {
...
void QuadPlane::update_throttle_suppression(void)
{
    ...
    if ((plane.get_throttle_input() != 0) || (plane.air_mode == AirMode::ON)) {
        return;
    }
...
void QuadPlane::update_throttle_mix(void)
{
    ...
    if ((plane.control_mode == &plane.mode_qstabilize) || (plane.control_mode == &plane.mode_qacro)) {
        // manual throttle
        if ((plane.get_throttle_input() <= 0) && (plane.air_mode == AirMode::OFF)) {
            attitude_control->set_throttle_mix_min();
        } else {
