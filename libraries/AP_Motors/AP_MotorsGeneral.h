/// @file	AP_MotorsGeneral.h
/// @brief	Motor control class for arbitrary vectored thrust configurations
#pragma once

#include <AP_Common/AP_Common.h>
#include <AP_Math/AP_Math.h>           // ArduPilot Vector/Matrix math Library
...
/// @class      AP_MotorsGeneral
class AP_MotorsGeneral : public AP_MotorsMulticopter {
public:

    /// Constructor
    AP_MotorsGeneral(uint16_t loop_rate, uint16_t speed_hz = AP_MOTORS_SPEED_DEFAULT) :
    ...
    // TODO: return the roll factor of any motor, this is used for tilt rotors and tail sitters
    // using copter motors for forward flight
    float               get_roll_factor(uint8_t i) override { return 0.0f; }

    // return the location and direction of any motor
    Vector3f            get_loc(uint8_t i) { return _loc[i]; }
    Vector3f            get_dir(uint8_t i) { return _dir[i]; }
...
protected:
    ...
    Vector3f            _loc[AP_MOTORS_MAX_NUM_MOTORS]; // each motor's location
    Vector3f            _dir[AP_MOTORS_MAX_NUM_MOTORS]; // each motor's thrust direction
    //TODO: _yaw_factor is currently used by the 2 NoYawTorque frame types to disable use of motor torque for yaw control
    // this becomes a continuous instead of binary decision in the general case where yaw torques are (possibly) constrained and balanced
    // the same as for roll and pitch
    float               _yaw_factor[AP_MOTORS_MAX_NUM_MOTORS]; // each motor's rotation direction
